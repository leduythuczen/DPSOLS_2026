import subprocess
import re
from parameter import ABC_LIB_PATH, USE_MULTICORE
...
def get_QoR(command_string, design_path, qor_mode="combined", verbose=0):
    ...
    try:
        ...
        # 🔥 Save to cache (SAFE for multicore)
        cached_qor[command_string] = QoR

        # ⚠️ Avoid file write conflicts in multiprocessing
        ...
        if verbose:
            print(
                f"=======[QoR mode={qor_mode}] "
                f"LUT={lut}/{ref_lut}, Levels={levels}/{ref_levels} "
                f"→ QoR={QoR:.4f}, improve {((2.0 - QoR) * 50):.1f}"
            )
